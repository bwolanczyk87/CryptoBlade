...
{
    public class BybitFuturesRestClient : IFuturesRestClient
    {
        private readonly IBybitRestClient m_bybitRestClient;
        private readonly Category m_category;
        private readonly ILogger<BybitFuturesRestClient> m_logger;
        private readonly IOptions<BybitFuturesRestClientOptions> m_options;
        ...
        public BybitFuturesRestClient(IOptions<BybitFuturesRestClientOptions> options,
            IOptions<TradingBotOptions> tradingBotOptions,
            IBybitRestClient bybitRestClient,
            ILogger<BybitFuturesRestClient> logger)
        {
            m_options = options;
                ...
                try
                {
                    var res = await ExchangePolicies<BybitOrderId>.RetryTooManyVisits
                        .ExecuteAsync(async () => await m_bybitRestClient.V5Api.Trading.PlaceOrderAsync(
                            ...
                            stopLossTakeProfitMode: request.StopLossTakeProfitMode,
                            stopLossLimitPrice: request.StopLossLimitPrice,
                            stopLoss: request.StopLossPrice,
                            stopLossOrderType: request.StopLossOrderType,
                            stopLossTriggerBy: request.SlTriggerBy,
                            takeProfitLimitPrice: request.TakeProfitLimitPrice,
                            takeProfit: request.TakeProfitPrice,
                            takeProfitOrderType: request.TakeProfitOrderType,
                            takeProfitTriggerBy: request.TpTriggerBy,
        ...
        {
            var stopRes = await ExchangePolicies.RetryForever.ExecuteAsync(
                async () =>
