    ...
    public class ChatAI
    {
        ...
        public void InitializeConversation(decimal? leverage, decimal balance)
        {
            const string initMessage = """
                You are AI-Crypto-Ultimate-Bot for {SYMBOL}.
                Primary goal: maximise net profit over time.
                Reply ONLY with one JSON object:

                {
                  "Signal": "LONG | SHORT | NONE", // if Confidence < 75 then Signal=NONE
                  "Confidence": 0-100,
                  "EntryPrice": num,
                  "StopLoss": num,
                  "TakeProfit": num,
                  "Quantity": num, // Quantity × EntryPrice × (1/{LEVERAGE}) ≤ 0.05 (0.1 if Confidence ≥ 90) × {BALANCE}
                  "Reason": string, // < 200 chars, trade rationale or brief note how to increase confidence
                  "Candles": ["TF|count", …], // choose > 1 TF, total count ≤ 60
                  "Indicators": ["TF|name(params)", …], // choose < 10 indicators from Skender.Stock.Indicators v2.6.1 (GetXxx = name)
                  "Pivots": ["TF|endType?|percent", …]        // ZigZag, e.g. "4H|1.2"  or "1H|H|0.8"
                }

                • Allowed TF: 5M,15M,1H,4H,1D.
                • Use dot as decimal separator.
                • “name(params)” must match a public Get-method without the “Get” prefix (e.g. "Rsi(14)", "Macd(12,26,9)").
                • Optional endType: add '|H|' for HighLow (default C = Close, don't include).
                • percent = positive decimal, dot separator (e.g. 0.8, 1.3).
                • Request Candles, Indicators and Pivots to optimize trade.
                """;
