...
{
    public class BybitRestDataClient
    {
        private readonly  IBybitRestClient _bybitRestClient;
        private readonly ILogger<BybitRestClient> _logger;
        public BybitRestDataClient(ILogger<BybitRestClient> logger)
        {
            ...
            CancellationToken cancel = default)
        {
            ...
            {
                _logger.LogError($"Failed to setup leverage. Max leverage is not set for {symbol.Name}");
                return false;
            }
            ...
            if (!leverageOk)
                _logger.LogError($"Failed to setup leverage. {leverageRes.Error?.Message}");
            ...
            CancellationToken cancel = default)
        {
            ...
            if (!modeOk)
                _logger.LogError($"Failed to setup position mode. {modeChange.Error?.Message}");
            ...
            CancellationToken cancel = default)
        {
            for (int attempt = 0; attempt < m_options.Value.PlaceOrderAttempts; attempt++)
            {
                _logger.LogDebug($"{symbol} Placing limit buy order for '{quantity}' @ '{price}'");
                ...
                if (orderStatusRes.GetResultOrError(out var orderStatus, out _))
                {
                    ...
                    {
                        _logger.LogDebug($"{symbol}: Buy order was cancelled. Adjusting price.");
                    ...
                    _logger.LogInformation($"{symbol} Buy order placed for '{quantity}' @ '{price}'");
                    return true;
                }

                _logger.LogInformation($"{symbol} Error getting order status: {orderStatusRes.Error}");
                return false;
            }

            _logger.LogInformation($"{symbol} could not place buy order.");
            ...
            CancellationToken cancel = default)
        {
            for (int attempt = 0; attempt < m_options.Value.PlaceOrderAttempts; attempt++)
            {
                ...
                if (orderStatusRes.GetResultOrError(out var orderStatus, out _))
                {
                    ...
                    if (order != null && order.Status == OrderStatus.Cancelled)
                    {
                        _logger.LogDebug($"{symbol} Sell order was cancelled. Adjusting price.");
                        var orderBook = await ExchangePolicies<BybitOrderbook>
                            .RetryTooManyVisits
                            .ExecuteAsync(async () =>
                                await m_bybitRestClient.V5Api.ExchangeData.GetOrderbookAsync(m_category,
                    ...
                    _logger.LogInformation($"{symbol} Sell order placed for '{quantity}' @ '{price}'");
                    return true;
                }
            ...
            _logger.LogInformation($"{symbol} could not place sell order.");
            return false;
        }
            ...
            CancellationToken cancel = default)
        {
            ...
            {
                _logger.LogInformation($"{symbol} Failed to place long take profit order: {error}");
                return false;
            }
            ...
            if (orderStatusRes.GetResultOrError(out var orderStatus, out _))
            {
                ...
                if (order != null && order.Status == OrderStatus.Cancelled)
                {
                    _logger.LogDebug($"{symbol} long take profit order was cancelled.");
                    return false;
                }
            }
            ...
            CancellationToken cancel = default)
        {
            ...
            {
                _logger.LogInformation($"{symbol} Failed to place short take profit order: {error}");
                return false;
            }
            ...
            if (orderStatusRes.GetResultOrError(out var orderStatus, out _))
            {
                ...
                if (order != null && order.Status == OrderStatus.Cancelled)
                {
                    _logger.LogError($"{symbol} short take profit order was cancelled.");
                    return false;
                }
            }
            ...
            StopLossTakeProfitMode? stopLossTakeProfitMode = StopLossTakeProfitMode.Full, CancellationToken cancel = default)
        {
            ...
            if (!stopRes.Success)
            {
                _logger.LogInformation($"{symbol}: Bybit SetTradingStop responded with success=false. Error: {stopRes.Error?.Message}");
            ...
            var positions = await ExchangePolicies.RetryForever.ExecuteAsync(async () =>
            {
                ...
                while (true)
                {
                    ...
                    foreach (var bybitPosition in data.List)
                    {
                        ...
                        if (position == null)
                            _logger.LogWarning($"Could not convert position for symbol: {bybitPosition.Symbol}");
                        else
